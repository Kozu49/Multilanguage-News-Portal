                    ...
                    <div class="table-responsive">
                      <table class="table table-bordered">
                        ...
                        @foreach($photos as $photo)
                          ...
                          <td> {{$i++}} </td>
                            <td> {{$photo->title}} </td>
                            <td><img src="{{asset('image/photogallery/'.$photo->photo)}}" style="width:50px; height:50px;"></td>
                            <td> 
                            @if($photo->type==1)
                            ...
                            <td>  
                                ...
                                <a href="{{route('delete.photo',$photo->id)}}" onclick="return confirm('Are you sure?')" class="btn btn-danger">Delete</a>
                            </td>                           
                          </tr>
                          @endforeach
                        </tbody>
                      </table>
                      {{$photos->links('pagination-link')}}
                    </div>
                  </div>
                </div>
              </div>
